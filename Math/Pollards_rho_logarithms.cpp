...
// Only works(?) when g is a generator and N is a prime number
...
int findInverse(int x, int mod) {
	...
	if (interRes[0] != 1) {
		return -1;
		// gcd is not 1
		//cout << "GCD is not 1, exiting..";
		//throw invalid_argument("X and mod are not coprime!");
	}
	return (extendEuclid(x, mod)[1] + mod) % mod;
}
...
int pollardsRhoDL(int g, int alpha, int order, int N, int x = 1, int a = 0, int b = 0) {
	// init vals
